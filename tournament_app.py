...
class SetupFrame(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # --- NUEVO: Botón para Corregir/Eliminar ---
    ...
    def correct_team(self):
        """
        Elimina el equipo seleccionado y devuelve sus datos a las cajas de texto
        para que el usuario pueda corregir el error rápidamente.
        """
        ...
        # El formato en la lista es: " {name}  [ID: {team_id}]"
        # Usamos parsing simple para extraer los datos
        try:
            # Separamos por el marcador de ID
            parts = item_text.split("  [ID: ")
            name = parts[0].strip()
            team_id = parts[1].rstrip("]") # Quitamos el ']' final
            
            # 1. Eliminar del diccionario de datos
            if team_id in self.controller.teams:
            ...
            self.error_label.config(text="ℹ️ Equipo eliminado. Corrige los datos y agrégalo de nuevo.")
            
        except Exception:
            self.error_label.config(text="⚠️ Error al procesar la selección.")

    def start_tournament(self):
        """Valida que haya suficientes equipos e inicia la lógica del torneo."""
        ...
        # Generar primera ronda y cambiar pantalla
...
class MatchFrame(ttk.Frame):
    ...
    def display_matches(self):
        """Genera los widgets (etiquetas e inputs) para cada partido de la ronda."""
        ...
        self.canvas.yview_moveto(0) # Volver al inicio del scroll
        ...
        for i, (team1_id, team2_id) in enumerate(self.controller.current_matches):
            ...
            # Nombre Equipo 2 (Manejo de BYE/Libre)
            if team2_id == "BYE":
                t2_text = "--- LIBRE ---"
                # Ahora permitimos edición manual del puntaje para el equipo que tiene BYE
                s2_ent.insert(0, "0"); s2_ent.config(state="disabled") # El rival 'LIBRE' se queda en 0
                
                # Registramos el widget s1 para leerlo en submit_scores, y None para s2
    ...
    def submit_scores(self):
        """Procesa los puntajes ingresados, actualiza totales y pasa de ronda."""
        ...
        try:
            for (entry1, entry2, team1_id, team2_id) in self.controller.match_entry_widgets:
                # Si es un BYE, procesamos solo el puntaje del equipo 1
                if team2_id == "BYE":
                    s1 = entry1.get()
                    if not s1:
            ...
            self.error_label.config(text="")
            self.controller.generate_pairings() # Siguiente ronda
            self.display_matches()
        except ValueError:
...
class StandingsFrame(ttk.Frame):
    ...
    Muestra la tabla final al terminar el torneo.
    """
    def __init__(self, parent, controller):
        super().__init__(parent, style="Main.TFrame")
        self.controller = controller
    ...
    def display_standings(self):
        """Refresca la tabla con los datos finales ordenados."""
...
class TournamentApp(tk.Tk):
    ...
    Maneja la ventana principal, la navegación entre vistas y la lógica del torneo.
    """
    def __init__(self):
        super().__init__()
        self.title("Gestor de Torneos - Trugo")
        
        # --- Configuración de Tamaño Dinámico ---
        screen_width = self.winfo_screenwidth()
        screen_height = self.winfo_screenheight()
        
        # Usar 85% del ancho y 80% del alto de la pantalla
        ...
        # --- Variables de Estado ---
        self.teams = {}            # Diccionario {id: ObjetoTeam}
        self.current_round = 0     # Ronda actual (1, 2, 3...)
        self.current_matches = []  # Lista de tuplas (id1, id2)
        self.match_entry_widgets = [] # Referencias a los inputs para leer datos luego
        self.tournament_name = "Torneo_Trugo"

        # Aplicar estilos visuales
        self.setup_styles()

        # --- Sistema de Navegación (Container) ---
    ...
    def setup_styles(self):
        """Define los estilos personalizados para widgets (botones, inputs, etc)."""
        style = ttk.Style()
    ...
    def show_frame(self, cont):
        """Muestra una de las vistas (frames) y la actualiza si es necesario."""
    ...
    def reset_tournament(self):
        """Reinicia todos los datos para comenzar desde cero."""
        self.teams = {}
    ...
    def save_tournament_data(self):
        """
        Guarda el estado del torneo en un archivo .txt.
        Incluye datos legibles (reporte) y datos técnicos para poder cargar el torneo después.
        """
        try:
            # Limpiar nombre de archivo para evitar errores del sistema operativo
    ...
    def load_tournament(self):
        """Lee un archivo .txt generado por esta app y restaura el estado."""
    ...
    def generate_pairings(self):
        """
        Algoritmo Sistema Suizo Simplificado:
        1. Ordena equipos por puntaje.
        2. Asigna BYE (libre) si es impar.
        3. Empareja a los mejores disponibles que no hayan jugado entre sí.
        """
        self.current_round += 1
        new_matches = []
        
        # Ordenar equipos: Mayor puntaje primero
        sorted_teams = sorted(self.teams.values(), key=lambda t: t.total_points, reverse=True)
        ...
        if len(unpaired_teams) % 2 != 0:
            bye_team = None
            # Buscar desde el fondo (peor puntaje) alguien que no haya tenido BYE
            for team in reversed(unpaired_teams):
                if not team.received_bye:
                    bye_team = team
                    break
            # Si todos tuvieron BYE, dárselo al último de todas formas
            if bye_team is None:
                bye_team = unpaired_teams[-1]
                
            bye_team.received_bye = True
            # bye_team.total_points += 1  <-- ELIMINADO: Ya no se suman puntos automáticos
            new_matches.append((bye_team.id, "BYE"))
            unpaired_teams.remove(bye_team)

        # Generar emparejamientos
        while unpaired_teams:
            team1 = unpaired_teams.pop(0) # El mejor clasificado disponible
            found_opponent = False
            
            # Buscar su mejor rival posible (que no haya jugado antes)
            ...
            # Si ya jugó con todos los disponibles (caso raro), juega con el siguiente
